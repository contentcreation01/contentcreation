...
## 9. Strategy Crack War Room — Architecture & Agent Memory

This section documents the advanced dual-mode dashboard and its viral intelligence engine.

### 9.1 Two Dashboard Modes

| Mode | Entry Point | Input | Output |
|---|---|---|---|
| **Mode A — Single Video Analysis** | "Analyze Single Video" button | 1 video URL | Full 20-asset project folder |
| **Mode B — Strategy Crack War Room** | "Strategy Crack War Room" button | 2–6 video URLs + prompt | Forensic breakdown + 5 Section B deliverables |

### 9.2 Strategy Crack Session Persistence

Every War Room session is auto-saved with NO user action required:

- **Base Directory**: `Instagram/Reels/Strategy_Cracks/`
- **Folder Naming**: `YYYYMMDD_HHMMSS_ms_<CTA>_Session/`
  - Example: `20260920_183045_123_ULTRON_Session/`
- **Numbered Assets Saved Per Session**:
  1. `01_strategy_forensic_breakdown.md` — The full Section A strategic intelligence report
  2. `02_teleprompter_script_<LANG>.md` — The ready-to-film script (in selected language)
  3. `03_capcut_edl_timeline.edl` — CapCut-native cut markers
  4. `04_google_flow_higsfield_prompts.md` — AI video prompts for both tools
  5. `05_caption_manychat_funnel.md` — Caption copy + ManyChat DM automation blueprint
  6. `06_hook_rewrites.md` — 3 viral hook rewrites with CapCut SFX cues
  7. `SESSION_INDEX.md` — One-line summary for quick search/reference
  8. `LATEST_STRATEGY_CRACK.md` — Updated in Sagar Builds project folder (cross-reference only)

### 9.3 Four-Language Script Engine

The War Room generates teleprompter scripts in 4 languages. Language is set via:
- **API parameter**: `language` field in POST to `/api/strategy-crack`
- **Auto-detection**: If the user types "tanglish", "mixed", or "tamil" in the prompt, language auto-switches
- **Client-side switching**: JS function `switchScriptLanguageView()` switches between pre-generated languages without re-hitting the API

| Language Key | Display Name | Script Character Set |
|---|---|---|
| `english` | English | Latin only |
| `tanglish` | Tanglish | Latin (phonetic Tamil romanization) |
| `mixed` | Mixed Tamil-English | Tamil Unicode + Latin |
| `tamil` | Native Tamil | Tamil Unicode script only |

### 9.4 Section B Deliverables Selector

In Mode B, the user explicitly selects which deliverables to generate BEFORE submitting:
- `script` — Teleprompter script (in all 4 languages)
- `edl` — CapCut EDL timeline markers
- `ai` — Google Flow & Higsfield AI video prompts
- `funnel` — Caption copy + ManyChat funnel
- `hooks` — 3 viral hook rewrites

The API returns only the selected deliverables to keep the output focused.

### 9.5 Sagar Builds — Isolated Reference Project

The Sagar Builds project (`Instagram/Reels/sagar_builds - Building Ultron in public (DayJ17OTwvx)/`) is kept **completely separate** from Luke/Chandler/Cindy strategy sessions. It is used as a **personal re-creation target** (the "Cindy role" applied to Sagar's Ultron setup). Cross-referencing is done only by writing a `LATEST_STRATEGY_CRACK.md` pointer file — the project's existing 20 assets are never modified by the War Room.

### 9.6 CTA Keyword Auto-Detection

The engine auto-extracts a CTA keyword from the user's prompt:
- Default: `JARVIS`
- If prompt contains `ultron`, `phone`, `adb`, `device`: → `ULTRON`
- If prompt contains `cta: KEYWORD`: → extracted `KEYWORD` (uppercase)

This CTA is used in the caption, ManyChat trigger, EDL freeze-frame badge, and hook rewrites.

### 9.7 Windows & GitHub Readiness Standards

All code in this workspace must meet the following cross-platform standards:

1. **Path Handling**: Always use `os.path.join()` or `pathlib.Path()`. Never hardcode `/` separators.
2. **Python Executable**: Always use `sys.executable` when spawning subprocesses (not hardcoded `python3`).
3. **File Encoding**: Always open files with `encoding="utf-8"` — critical for Tamil Unicode scripts on Windows.
4. **Speech Synthesis**:
   - macOS: `say -v <voice> -r <rate> -o <file>.aiff <text>`
   - Windows: `powershell -Command "Add-Type -AssemblyName System.Speech; ..."` (SAPI5)
5. **OCR Helper**: `ocr_helper` binary is macOS-only. Windows falls back to caption/transcript data gracefully.
6. **Browser Launch**: Use `webbrowser.open()` — works on both platforms.
7. **Git**: Model weights (`*.bin`) and large media (`*.mp4`, `*.m4a`, `*.wav`) are in `.gitignore`. Only markdown reports, scripts, and code are committed.

### 9.8 Agent Memory — The Viral Evolution Framework

The foundational case study for this system:
- **Luke (@lukebuildsai)** → Posted an AI agent demo that went viral for its "Iron Man" sci-fi feel
- **Chandler (@chandlerintelligence)** → Recreated Luke's exact stack using FastMCP + local code. Went viral with builders.
- **Cindy (@cindiezhu)** → Recreated Chandler's setup. Removed terminal intimidation. Framed as a peer test. Hit **53% comment-to-like ratio**.

The Cindy Pattern: Same visual architecture + relatable framing + zero gatekeeping = breakthrough engagement.

**The user's role**: Apply this same pattern to their own Sagar Builds / Ultron phone automation setup — using their own physical devices as the "live proof" hook instead of a screen recording.
